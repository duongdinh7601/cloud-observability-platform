...
def parse_level(level: Optional[str]=None) -> Optional[LogLevel]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Invalid level '{level}'. Allowed: {allowed}"
        )
    
def resolve_cursor(cursor_ts: Optional[datetime]=None, cursor_id: Optional[int]=None):
    if cursor_ts is not None and cursor_id is not None:
        ...
        raise HTTPException(
                status_code=422,
                detail="Both cursor_ts and cursor_id is required"
                )